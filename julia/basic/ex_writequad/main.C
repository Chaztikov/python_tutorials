...
int main(int argc, char **argv)
{
    ...
    std::string system_var("system_var_0");
    ...
    system.add_vector("approx1");
    system.add_vector("approx2");
    system.add_vector("exact");
    system.add_vector("error1");
    system.add_vector("error2");

    equation_systems.init();

    equation_systems.get_system(system_name).project_solution(*exact_solution, libmesh_nullptr, equation_systems.parameters);
    ...
    system.get_dof_map().add_dirichlet_boundary(DirichletBoundary(bcids, variables_sys, zero_func));
    ...
    std::unique_ptr<NumericVector<Number>> scalarfield_node = integrate_exact(equation_systems);
