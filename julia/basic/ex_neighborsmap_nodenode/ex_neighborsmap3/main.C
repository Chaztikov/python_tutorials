...
Number kernel3(const Real &x) {
  ...
  val *= (27. - 120. * pow(absx, 2)) / 17.;
  return val * boundary_indicator(x) / horizon / 2;
}
  ...
  if (boundary_indicator(p, horizon_boundary_width)) {
    ...
    for (const auto &neighbor : patch) {
      ...
      for (auto qp : range(qrule->n_points())) {
        ...
        const Point &xi((qpoint[qp] - p));

        Real kernel_val_qp = kernel(xi) * JxW[qp];

        kernel_moment_0 += kernel_val_qp;

        for (auto i : range(ndimensions))
          kernel_moment_1(i) += kernel_val_qp * p.slice(i);
      }
    }
...
int main(int argc, char **argv) {
  ...
  double nelem_div_horizon = input_file("nelem_div_horizon", 2.0);
  ...
  // nelem = 32;
  h_elem = (xmax - xmin) * (double)(1. / (static_cast<double>(nelem)));
  ...
  //   std::cout << "\n Node " << node;
  //   std::cout << " elem neighbor count : " <<
  //   MapNodeToElemNeighbor[node].size()
  //             << std::endl;
  //   std::cout << " node neighbor count : " <<
  //   MapNodeToNodeNeighbor[node].size()
  //             << std::endl;
  // }
