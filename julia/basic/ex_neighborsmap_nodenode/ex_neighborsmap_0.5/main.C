...
int main(int argc, char **argv)
{
  ...
  fe_family = LAGRANGE;
  // Utility::string_to_enum<FEFamily>(input_file("fe_family",LAGRANGE));
  // Utility::string_to_enum<FEFamily>
  ...
  std::vector<Number> approx;
  // NumericVector<Number> output_approx(init.comm());
  ...
  // output_system.solution->add(output_approx);
    // output_system.project_solution(integrate_patch, libmesh_nullptr, equation_systems.parameters);
