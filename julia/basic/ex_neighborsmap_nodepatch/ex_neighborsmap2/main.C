...
// void compute_node_elem_neighbors_from_node_neighbors(const MeshBase &mesh,
// const Node *node,  const Elem *elem)
// {
...
void compute_node_elem_neighbors(const MeshBase &mesh, const Node *node,
                                 const Elem *elem) {
  // build patch
  libMesh::Patch patch(mesh.processor_id());
  patch.build_around_element(elem, std::pow(1 + 2 * horizon, ndimensions),
                             &Patch::add_point_neighbors);
  for (auto elem : patch) {
    MapNodeToElemNeighbor[node].emplace_back(elem);
  }
  // check that neighbors of patch are all here
  // for(const auto neighbor : patch)
  // {
  //   for (auto side : neighbor->side_index_range())
  //   {
  //     if ( patch.count(neighbor->neighbor_ptr(side) == libmesh_nullptr) > 0)

  //   }
  // }
}

void map_node_elem_neighbor(const MeshBase &mesh, bool use_nodes = 0) {
  ...
  for (const auto &node : mesh.node_ptr_range()) {
    ...
    if (use_nodes)
      ...
      // std::vector<const Elem *> &elemvec = MapNodeToElemNeighbor[node];
      // elemvec.reserve(patch.size());
      // elemvec.insert(patch.begin(),patch.end());

      std::vector<const Elem *> elemvec(patch.begin(), patch.end());
      MapNodeToElemNeighbor.insert(std::make_pair(node, elemvec));
      // libMesh::Patch patch(mesh.processor_id());
      // patch.build_around_element(elem,
      //                           std::pow(1+2*horizon , ndimensions),
      //                           &Patch::add_point_neighbors);
    }
  }
}
...
Number integrate_patch(const Node &node, const Parameters &parameters,
  ...
  int num_elem_neighbor = 0;
  int num_elem_neighbor_qp = 0;
  int num_elem_neighbor_qp_nzkernel = 0;
  ...
  if (boundary_indicator(p, horizon_boundary_width)) {
    ...
    for (const auto &neighbor : patch) {
      ...
      Real kernel_moment_1elem_0=0;

      for (auto qp : range(qrule->n_points())) {
        num_elem_neighbor_qp++;
        ...
        if (kernel_val_qp > 0)
          num_elem_neighbor_qp_nzkernel++;

        kernel_moment_0 += kernel_val_qp * JxW[qp];

        kernel_moment_1elem_0 += kernel_val_qp * JxW[qp];
    ...
    std::cout << "\n \n num_elem_neighbor " << num_elem_neighbor
              << "\n num_elem_neighbor_qp " << num_elem_neighbor_qp
              << "\n num_elem_neighbor_qp_nzkernel "
              << num_elem_neighbor_qp_nzkernel;
  }
...
int main(int argc, char **argv) {
  ...
  kernel_order = input_file("kernel_order", 1);
